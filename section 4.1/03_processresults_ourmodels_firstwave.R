...
for (i in 1:54){
 ...
 results <-  process_results(df_full, 
  ...
  write.csv(results$df_full, file = paste0('./nowcast_firstwave/results_model',i,'.csv'))
  write.csv(results$station_ave_df, file = paste0('./nowcast_firstwave/CANresults_model',i,'.csv'))
}


## We also need to process the full model in this way
## I'm importing the model fit from section 3.1 

load("../section 3.1/model1.RData")
...
write.csv(results$df_full, file = './nowcast_firstwave/results_fullmodel.csv')
write.csv(results$station_ave_df, file = './nowcast_firstwave/CANresults_fullmodel.csv')
